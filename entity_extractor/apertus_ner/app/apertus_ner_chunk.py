...
def query_weaviate(client, collection_name, entity_names, language, date=None, top_k=10):
    """
    ...
    for name in entity_names:
        ...
        if collection_name == PEOPLE_COLLECTION:
            query_args["return_properties"].extend(["name", "description", "identifier", "deathyear","birthyear"])
            ...
            if collection_name == PEOPLE_COLLECTION:
                ...
                if deathyear_str:
                    try:
                        ...
                        if deathyear < date - timedelta(days=7300):  
                            print(f"Failed Too Old Check: Person {candidate_name} died: {deathyear}, while document date is {date}")
                            continue  # Skip old death date
                        else: print(f"Passed Too Old Check: Person {candidate_name} died: {deathyear}, while document date is {date}")
                    except Exception:
                        print(f"failed to parse deathyear for candidate {candidate_name}")
                        pass  # If date parsing fails, don't filter out
                try:
                    birthyear = datetime(int(birthyear_str.split(".")[0]), 1,1)
                    if birthyear > date - timedelta(days=7300):  # 20 years before date
                        print(f"Failed Too Young Check: Person {candidate_name} was born: {birthyear}, while document date is {date}")
                        continue  # Skip old death date
                    else: print(f"Passed Too Young Check: Person {candidate_name} was born: {birthyear}, while document date is {date}")
                except Exception:
                    print(f"failed to parse birthyear for candidate {candidate_name}")
                    pass  # If date parsing fails, don't filter out
...
def disambiguate(nlp, text, text_date, client,  language, top_k=8):
    ...
    people_results = query_weaviate(client, PEOPLE_COLLECTION, entities["PER"], language, text_date, top_k)
    org_results = query_weaviate(client, ORG_COLLECTION, entities["ORG"], language, None, top_k)
    place_results = query_weaviate(client, GEO_COLLECTION, entities["LOC"], language, None, top_k )
    result = {
...
def ner_lang_chunks(chunks, language, date, client):
    results = []
    if language == "de":
    ...
    for chunk in chunks:
        for order, text in chunk.items():
            ...
            candidates = disambiguate(nlp,text,date,client,"de")
            if candidates['people']:
                ...
                if people.get("people"):
                    ...
                    print(f"LLM identified {len(entities.get("people"))} people along with the {len(entities.get("organizations"))} organizations and {len(entities.get("places"))} places in chunk {order}")
                    logging.info(f"LLM identified {len(entities.get("people"))} people along with the {len(entities.get("organizations"))} organizations and {len(entities.get("places"))} places in chunk {order}")
                else: 
            ...
            elif candidates['organizations'] or candidates['places']:
                results.append({'chunk_id': order, 'entities':candidates})
            else: 
                print(f"Matched no people for chunk {order}")
                logging.info(f"Matched no people for chunk {order}")
...
def definitive_id(doc_id, date):
    ...
    chunks = collection.query.fetch_objects(filters=filters, limit=1000)
    de_chunks = []
    fr_chunks = []
    it_chunks = []
    other_chunks = []
    ...
    client.close
    if de_chunks:
        results.extend(ner_lang_chunks(de_chunks, "de", date, client))
    if fr_chunks:
        results.extend(ner_lang_chunks(fr_chunks, "fr", date, client))
    if it_chunks:
       results.extend(ner_lang_chunks(it_chunks, "it", date, client))
    if other_chunks:
        results.extend(ner_lang_chunks(other_chunks, "other", date, client))
    client.close
    return results
    ...
    with tracker:
        ...
        try:
            ...
            for chunk_ents in doc_ents:
                ...
                t_count_people += count_people
                t_count_org += count_org
                t_count_geo += count_geo
                print(f"Extracted and stored {count_people} people, {count_org} organizations and {count_geo} places from chunk {chunk_ents['chunk_id']}")
                logging.info(f"Extracted and stored {count_people} people, {count_org} organizations and {count_geo} places from chunk {chunk_ents['chunk_id']}")
            elapsed = time.time() - start_time
            print(f"Extracted and stored {t_count_people} people, {t_count_org} organizations and {t_count_geo} places from document {doc_id} in {elapsed:.2f} seconds")
            logging.info(f"Extracted and stored {t_count_people} people, {t_count_org} organizations and {t_count_geo} places from document {doc_id} in {elapsed:.2f} seconds")
            return {"status": "success", "doc_id": doc_id, "result": f"Extracted and stored {t_count_people} people, {t_count_org} organizations and {t_count_geo} places from document {doc_id} in {elapsed:.2f} seconds"}
        except Exception as e:
